...
def get_enzyme(name):
    """Get enzyme or custom cutter."""
    from Bio import Restriction
    import re
    from Bio.Seq import Seq

    # ---- Custom OmniC Bridge cutter ----
    if name == "OmniC_Bridge":
        class OmniC_Bridge:
            site = "GGTTCGTCCATCGATC"
            size = len(site)
            fst5 = 0
            fst3 = 0
            pattern_fwd = re.compile(site)
            pattern_rev = re.compile(str(Seq(site).reverse_complement()))

            @classmethod
            def search(cls, sequence):
                if isinstance(sequence, Seq):
                    sequence = str(sequence)
                cut_sites = []
                # Forward strand
                for m in cls.pattern_fwd.finditer(sequence):
                    cut_sites.append(m.start() + 1)
                # Reverse complement strand
                for m in cls.pattern_rev.finditer(sequence):
                    cut_sites.append(m.start() + 1)
                cut_sites.sort()
                return cut_sites
        return OmniC_Bridge

    # ---- fallback to Biopython enzymes ----
    enz = getattr(Restriction, name, None)
    if enz is None:
        raise ValueError(f"Enzyme not found: {name}")
    if enz.cut_twice():
        raise NotImplementedError(
            f"Enzyme cuts twice, not currently supported: {name}"
        )
    return enz
